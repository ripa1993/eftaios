package it.polimi.ingsw.cg_8;

import java.io.File;

/**
 * Container for references to resource files
 * 
 * @author Simone
 * @version 1.0
 */
public class Resource {
	/**
	 * Reference to Fermi map image
	 */
	public static final String IMG_FERMI_MAP = "res" + File.separatorChar
			+ "images" + File.separatorChar + "fermi.png";
	/**
	 * Reference to Galvani map image
	 */
	public static final String IMG_GALVANI_MAP = "res" + File.separatorChar
			+ "images" + File.separatorChar + "galvani.png";
	/**
	 * Reference to Galilei map image
	 */
	public static final String IMG_GALILEI_MAP = "res" + File.separatorChar
			+ "images" + File.separatorChar + "galilei.png";
	/**
	 * Alien attack 1
	 */
	public static final String SOUND_ALIEN_ATTACK_1 = "res"
			+ File.separatorChar + "sounds" + File.separatorChar
			+ "alien_attack_1.wav";
	/**
	 * Alien attack 2
	 */
	public static final String SOUND_ALIEN_ATTACK_2 = "res"
			+ File.separatorChar + "sounds" + File.separatorChar
			+ "alien_attack_2.wav";
	/**
	 * Alien attack 3
	 */
	public static final String SOUND_ALIEN_ATTACK_3 = "res"
			+ File.separatorChar + "sounds" + File.separatorChar
			+ "alien_attack_3.wav";
	/**
	 * Death
	 */
	public static final String SOUND_DEATH = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "death.wav";
	/**
	 * Escape hatch
	 */
	public static final String SOUND_ESCAPE_HATCH = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "escape_hatch.wav";
	/**
	 * Human Attack 1
	 */
	public static final String SOUND_HUMAN_ATTACK_1 = "res"
			+ File.separatorChar + "sounds" + File.separatorChar
			+ "human_attack_1.wav";
	/**
	 * Human attack 2
	 */
	public static final String SOUND_HUMAN_ATTACK_2 = "res"
			+ File.separatorChar + "sounds" + File.separatorChar
			+ "human_attack_2.wav";
	/**
	 * Human attack 3
	 */
	public static final String SOUND_HUMAN_ATTACK_3 = "res"
			+ File.separatorChar + "sounds" + File.separatorChar
			+ "human_attack_3.wav";
	/**
	 * Noise
	 */
	public static final String SOUND_MOVEMENT_NOISE = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "noise.wav";
	/**
	 * Notification
	 */
	public static final String SOUND_NOTIFICATION = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "notification.wav";
	/**
	 * Shield
	 */
	public static final String SOUND_DEFENSE = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "shield.wav";
	/**
	 * Spotlight
	 */
	public static final String SOUND_SPOTLIGHT = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "spotlight.wav";
	/**
	 * Teleport
	 */
	public static final String SOUND_TELEPORT = "res" + File.separatorChar
			+ "sounds" + File.separatorChar + "teleport.wav";

}
